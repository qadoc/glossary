...
|:--|:--|
| [V 模型](V/V_Model.md) | ① 测试的各个阶段和开发的各个阶段相对应，**测试更具体**。<br/>② 仅测试被测系统的程序，仅包含动态测试。<br/>③ 测试在开发活动之后进行。 |
| [W 模型](W/W_Model.md) | ① **测试尽早介入开发活动**，提前发现问题，降低缺陷修复成本。<br/>② 测试对象从程序扩展到文档，增加了静态测试，使得测试更全面。<br/>③ 测试活动与开发活动同步进行，每个开发阶段都对应一个测试阶段。 |
| [H 模型](H/H_Model.md) | ① **独立测试“微循环”流程**，与其他流程并行，贯穿整个产品周期。<br/>② **测试对象可以是任何需要被测试的东西。**<br/>③ 测试不仅是测试执行，还包括**测试准备和测试准入**。<br/>④ 测试活动要尽早准备、尽早执行。 |
